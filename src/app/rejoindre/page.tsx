...
export default function RejoindrePage() {
  ...
  const inputCls = "w-full bg-[#1A1612]/5 border border-[#1A1612]/12 rounded-xl px-4 py-3 font-hanken text-sm text-[#1A1612] placeholder-[#1A1612]/25 focus:outline-none focus:border-[#FF3D7F]/50 transition-colors";
    ...
    <div className="min-h-screen bg-[#FBF9F5] text-[#1A1612]">
      ...
      <section className="relative pt-32 pb-16 px-6 overflow-hidden">
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[700px] h-[400px] rounded-full blur-[100px] opacity-20 pointer-events-none"
          style={{ background: "radial-gradient(ellipse, #FF3D7F 0%, #6D2DB5 70%, transparent 100%)" }} />
        <div className="relative max-w-3xl mx-auto text-center">
          ...
          <h1 className="font-fraunces text-5xl md:text-6xl font-light leading-[1.1] mb-6">
            Rejoins{" "}
            <span className="italic text-[#1A1612]">Spectrum.</span>
          </h1>
          <p className="font-hanken text-lg text-[#1A1612]/60 max-w-2xl mx-auto leading-relaxed">
      ...
      <section className="py-10 px-6">
        <div className="max-w-3xl mx-auto">
          <p className="font-mono text-[11px] tracking-widest uppercase text-[#1A1612]/35 text-center mb-6">Qui peut rejoindre ?</p>
          <div className="flex flex-wrap justify-center gap-3">
            {PROFILES.map(p => (
              <div key={p.label}
                className="flex items-center gap-2 px-4 py-2 rounded-full border border-[#1A1612]/10 bg-[#1A1612]/[0.03] font-hanken text-sm text-[#1A1612]/70">
                <span>{p.emoji}</span> {p.label}
              </div>
            ))}
          </div>
          <p className="font-hanken text-sm text-[#1A1612]/40 text-center mt-5">
        ...
        <div className="max-w-xl mx-auto">
          ...
          {done ? (
            <div className="text-center py-16">
              ...
              <p className="font-hanken text-[#1A1612]/55 mb-8 leading-relaxed max-w-sm mx-auto">
                Merci {form.name.split(" ")[0]} ! On revient vers toi avant le 26 juin.
                Prépare-toi — la communauté t&apos;attend.
              </p>
              <Link href="/"
                className="inline-flex items-center gap-2 font-hanken text-sm text-[#FF3D7F] hover:text-[#FF3D7F]/80 transition-colors">
                ← Retour à l&apos;accueil
              </Link>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-mono text-[10px] uppercase tracking-widest text-[#1A1612]/35 mb-2">Prénom & Nom *</label>
                    <input value={form.name} onChange={f("name")} placeholder="Sasha Martin" className={inputCls} />
                  </div>
                  <div>
                    <label className="block font-mono text-[10px] uppercase tracking-widest text-[#1A1612]/35 mb-2">Email *</label>
                    <input type="email" value={form.email} onChange={f("email")} placeholder="sasha@email.com" className={inputCls} />
                  </div>
                </div>

                <div>
                  <label className="block font-mono text-[10px] uppercase tracking-widest text-[#1A1612]/35 mb-2">Ton activité *</label>
                  <select value={form.activity_type} onChange={f("activity_type")} className={inputCls}>
                    ...
                    {ACTIVITY_TYPES.map(a => (
                      <option key={a.value} value={a.value} className="bg-[#FBF9F5]">{a.label}</option>
                    ))}
                  </select>
                </div>

                <div>
                  <label className="block font-mono text-[10px] uppercase tracking-widest text-[#1A1612]/35 mb-2">Présente-toi en quelques mots</label>
                  <textarea
                    rows={4}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-mono text-[10px] uppercase tracking-widest text-[#1A1612]/35 mb-2">Instagram</label>
                    <input value={form.instagram} onChange={f("instagram")} placeholder="@ton_compte" className={inputCls} />
                  </div>
                  <div>
                    <label className="block font-mono text-[10px] uppercase tracking-widest text-[#1A1612]/35 mb-2">Site web</label>
                ...
                <label className="flex items-start gap-3 cursor-pointer group pt-1">
                  <input
                    ...
                    className="mt-0.5 w-4 h-4 rounded accent-[#FF3D7F]"
                  />
                  <span className="font-hanken text-sm text-[#1A1612]/55 group-hover:text-[#1A1612]/75 transition-colors leading-relaxed">
                ...
                <button
                  type="submit"
                  disabled={sending}
                  className="w-full flex items-center justify-center gap-2 py-4 rounded-xl bg-[#FF3D7F] text-white font-hanken font-semibold text-base hover:bg-[#FF3D7F]/85 hover:scale-[1.01] transition-all duration-200 shadow-lg shadow-[#FF3D7F]/20 disabled:opacity-50 disabled:scale-100 mt-2"
                >
                  {sending ? "Envoi en cours…" : <>Rejoindre Spectrum <ArrowRight size={16} /></>}
                </button>

                <p className="font-hanken text-xs text-[#1A1612]/25 text-center">
      ...
      {/* Already a vendor? */}
      <section className="py-10 px-6 border-t border-[#1A1612]/6">
        <div className="max-w-xl mx-auto text-center">
          <p className="font-hanken text-sm text-[#1A1612]/40 mb-3">
