...
export default function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!article) return (
    <div className="min-h-screen bg-[#FBF9F5] text-[#1A1612] flex flex-col items-center justify-center gap-4">
      <p className="font-fraunces text-2xl">Article introuvable</p>
      <Link href="/media" className="font-hanken text-[#FF3D7F] hover:underline">← Retour au média</Link>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F5] text-[#1A1612]">
      ...
      <article className="max-w-3xl mx-auto px-6 pt-28 pb-24">
        <Link href="/media" className="inline-flex items-center gap-2 text-[#1A1612]/40 hover:text-[#FF3D7F] transition-colors font-hanken text-sm mb-8">
        ...
        <div className="flex items-center gap-3 mb-4">
          <span className="font-mono text-[10px] text-[#FF3D7F] uppercase tracking-widest border border-[#FF3D7F]/30 px-2 py-0.5 rounded-full">
            {article.category}
          </span>
        ...
        {article.excerpt_fr && (
          <p className="font-hanken text-lg text-[#1A1612]/60 mb-8 leading-relaxed border-l-2 border-[#FF3D7F]/40 pl-4">
            {article.excerpt_fr}
          </p>
        )}

        <div
          className="font-hanken text-[#1A1612]/80 leading-relaxed prose prose-invert max-w-none"
        ...
        {article.tags && article.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-12 pt-8 border-t border-[#1A1612]/8">
            {article.tags.map(tag => (
              <span key={tag} className="font-mono text-[11px] bg-[#1A1612]/5 text-[#1A1612]/40 px-3 py-1 rounded-full">
                #{tag}
              </span>
            ))}
          </div>
