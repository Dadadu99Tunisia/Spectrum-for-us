...
function StripeForm({ totalCents, onSuccess, onBack }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="p-4 rounded-xl border border-[#1A1612]/10 bg-[#1A1612]/[0.02] flex items-start gap-3">
        <Lock size={14} className="text-[#1C9C95] mt-0.5 shrink-0" />
        <p className="font-hanken text-xs text-[#1A1612]/50 leading-relaxed">
          Paiement sécurisé via <strong className="text-[#1A1612]/70">Stripe</strong>. Tes données bancaires sont chiffrées et ne nous parviennent jamais.
        </p>
      </div>

      <div className="rounded-xl border border-[#1A1612]/15 p-4 bg-[#1A1612]/[0.02]">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-4 rounded-full bg-[#FF3D7F] text-white font-hanken font-semibold text-base hover:brightness-110 transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
      >
        <Lock size={14} />
      ...
      <button type="button" onClick={onBack}
        className="w-full text-center font-mono text-xs text-[#1A1612]/25 hover:text-[#1A1612]/50 transition-colors">
  ...
  if (confirmed) return (
    <div className="min-h-screen bg-[#FBF9F5] flex items-center justify-center px-6">
      <div className="text-center max-w-md">
        ...
        <h1 className="font-fraunces text-4xl text-[#1A1612] mb-3">Commande confirmée ✦</h1>
        {orderId && <p className="font-mono text-xs text-[#1A1612]/30 mb-4">#{orderId.slice(0, 8).toUpperCase()}</p>}
        <p className="font-hanken text-[#1A1612]/55 mb-8 leading-relaxed">
  ...
  if (items.length === 0 && !confirmed) return (
    <div className="min-h-screen bg-[#FBF9F5] flex items-center justify-center px-6">
      <div className="text-center">
        <h2 className="font-fraunces text-2xl text-[#1A1612] mb-4">Ton panier est vide</h2>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-2 mb-10 flex-wrap">
            {STEPS.map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`flex items-center gap-2 transition-colors ${s === step ? "text-[#1A1612]" : STEPS.indexOf(step) > i ? "text-[#1C9C95]" : "text-[#1A1612]/25"}`}>
                  <span className={`w-6 h-6 rounded-full border text-xs font-mono flex items-center justify-center transition-all ${
                    s === step ? "border-[#FF3D7F] text-[#FF3D7F]"
                ...
                {i < STEPS.length - 1 && <div className={`w-8 h-px ${STEPS.indexOf(step) > i ? "bg-[#1C9C95]" : "bg-[#1A1612]/15"}`} />}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
              ...
              {step === "Livraison" && (
                <>
                  ...
                  <div>
                    ...
                    <select value={form.country} onChange={(e) => set("country", e.target.value)}
                      ...
                      {["France", "Belgique", "Suisse", "Canada", "Luxembourg", "Autre"].map((c) => (
                        <option key={c} value={c} className="bg-[#FBF9F5]">{c}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-4 rounded-full bg-[#FF3D7F] text-white font-hanken font-semibold hover:brightness-110 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                  ...
                  {clientSecret && stripeReady ? (
                    ...
                    <div className="space-y-4">
                      <div className="p-5 rounded-xl border border-amber-400/20 bg-amber-400/5">
                        ...
                        <p className="font-hanken text-sm text-[#1A1612]/60 leading-relaxed">
                          La clé publique Stripe est configurée ✓<br />
                          Pour activer les vrais paiements, ajoute dans <code className="text-[#FF3D7F] font-mono text-xs">.env.local</code> :
                        </p>
                        <pre className="mt-3 p-3 bg-[#1A1612]/5 rounded-lg text-xs font-mono text-[#1A1612]">
                          STRIPE_SECRET_KEY=sk_live_...
                        </pre>
                      </div>
                      <button onClick={() => setStep("Livraison")} className="flex items-center gap-2 font-mono text-xs text-[#1A1612]/30 hover:text-[#1A1612]/60 transition-colors">
            ...
            <div className="lg:col-span-2">
              <div className="sticky top-24 rounded-2xl border border-[#1A1612]/10 bg-[#1A1612]/[0.025] p-5">
                <div className="prism-line mb-4" />
                <h2 className="font-bricolage font-bold text-[#1A1612] mb-4">Récapitulatif</h2>
                <div className="space-y-2 mb-4 max-h-48 overflow-y-auto pr-1">
                  {items.map((item) => (
                    <div key={item.id} className="flex justify-between gap-2 text-sm">
                      <span className="font-hanken text-[#1A1612]/70 truncate">{item.name} ×{item.quantity}</span>
                      <span className="font-mono text-[#1A1612] shrink-0">{(item.price * item.quantity).toFixed(2)} €</span>
                    </div>
                  ))}
                </div>
                <div className="border-t border-[#1A1612]/10 pt-3 mb-1 flex justify-between text-sm font-hanken text-[#1A1612]/50">
                  <span>Frais de port</span><span className="text-[#1C9C95]">Calculés</span>
                </div>
                <div className="flex justify-between font-bricolage font-bold text-[#1A1612] text-lg">
                  <span>Total</span>
