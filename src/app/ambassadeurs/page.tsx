  ...
  return (
    <div className="min-h-screen bg-[#FBF9F5] text-[#1A1612]">
      ...
      <section className="pt-32 pb-16 px-6 text-center max-w-3xl mx-auto">
        <p className="font-mono text-xs tracking-widest text-[#FF3D7F] uppercase mb-4">Programme Ambassadeur·rice</p>
        <h1 className="font-fraunces text-5xl md:text-6xl font-light mb-4">
          Partage. Gagne.<br /><span className="text-[#FF3D7F]">Fais grandir</span> la communauté.
        </h1>
        <p className="font-hanken text-[#1A1612]/60 text-lg mb-8">
          Partage Spectrum For Us et touche <strong className="text-[#1A1612]">10% de commission</strong> sur chaque vente générée via ton lien.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12 text-left">
          {[
            ...
            <div key={s.step} className="p-5 rounded-2xl border border-[#1A1612]/8 bg-[#1A1612]/[0.02]">
              <span className="font-mono text-3xl font-bold text-[#FF3D7F]/30">{s.step}</span>
              <h3 className="font-fraunces text-lg mt-2 mb-1">{s.title}</h3>
              <p className="font-hanken text-sm text-[#1A1612]/50">{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="max-w-2xl mx-auto px-6 pb-24">
        {loading ? (
          <div className="h-32 rounded-2xl bg-[#1A1612]/5 animate-pulse" />
        ) : !user ? (
          <div className="text-center p-8 rounded-2xl border border-[#1A1612]/10">
            <p className="font-hanken text-[#1A1612]/60 mb-4">Connecte-toi pour rejoindre le programme.</p>
            <Button href="/auth?mode=ambassador">Se connecter</Button>
          </div>
        ) : !ambassador ? (
          <div className="text-center p-8 rounded-2xl border border-[#FF3D7F]/20 bg-[#FF3D7F]/5">
            <h2 className="font-fraunces text-2xl mb-2">Prêt·e à rejoindre ?</h2>
            <p className="font-hanken text-[#1A1612]/60 mb-6">Ton code personnalisé sera créé instantanément.</p>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={s.label} className="p-4 rounded-xl border border-[#1A1612]/8 text-center">
                  <s.icon size={18} className="text-[#FF3D7F] mx-auto mb-2" />
                  <p className="font-fraunces text-xl">{s.value}</p>
                  <p className="font-mono text-[10px] text-[#1A1612]/40 mt-0.5">{s.label}</p>
                </div>
              ))}
            </div>

            {/* Code */}
            <div className="p-5 rounded-2xl border border-[#1A1612]/10 bg-[#1A1612]/[0.02]">
              <p className="font-mono text-xs text-[#1A1612]/40 uppercase tracking-widest mb-2">Ton lien ambassadeur·rice</p>
              <div className="flex items-center gap-3">
                <code className="flex-1 font-mono text-sm text-[#FF3D7F] bg-[#FF3D7F]/5 px-3 py-2 rounded-lg truncate">
                  spectrumforus.com?ref={ambassador.referral_code}
                </code>
                <button
                  onClick={copyCode}
                  className="p-2 rounded-lg border border-[#1A1612]/10 text-[#1A1612]/60 hover:text-[#FF3D7F] hover:border-[#FF3D7F]/30 transition-all"
                >
            ...
            {referrals.length > 0 && (
              <div className="rounded-2xl border border-[#1A1612]/8 overflow-hidden">
                <div className="p-4 border-b border-[#1A1612]/8">
                  ...
                  {referrals.map(r => (
                    <div key={r.id} className="flex items-center justify-between p-4">
                      <div>
                        ...
                        <p className="font-mono text-[10px] text-[#1A1612]/40">{r.status}</p>
                      </div>
                      <span className="font-mono text-sm text-[#FF3D7F]">+{r.commission_amount.toFixed(2)}€</span>
                    </div>
                  ))}
                </div>
              </div>
