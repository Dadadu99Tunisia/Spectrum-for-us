    ...
    <span className="inline-flex items-center gap-1 font-mono text-[10px]" style={{ color: colors[score] ?? "#1A1612" }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <span key={i} className="w-1.5 h-1.5 rounded-full inline-block transition-all"
          style={{ background: i < score ? (colors[score] ?? "#1A1612") : "rgba(243,234,219,.12)" }} />
      ))}
...
function PipelineBar({ contacts }: { contacts: Contact[] }) {
  ...
  return (
    <div className="bg-[#1c1235] border border-[#1A1612]/8 rounded-2xl p-5">
      {/* Bar */}
...
function ContactPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div ref={panelRef}
        className="relative z-10 w-full max-w-[420px] bg-[#1a1030] border-l border-[#1A1612]/8 flex flex-col h-full shadow-2xl"
        ...
        return (
          <button key={seg.id} onClick={() => setSegment(seg.id)}
            ...
            <span className={`font-mono text-[10px] px-1.5 py-0.5 rounded-full ${
              active ? "bg-[#1A1612]/20" : "bg-[#1A1612]/8 text-[#1A1612]/35"
            }`}>
              {count}
            </span>
          </button>
...
export default function CRMPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Goal progress ── */}
      <div className="bg-[#1c1235] border border-[#1A1612]/8 rounded-2xl p-5 space-y-3">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
          ...
          <div className="bg-[#1c1235] border border-[#1A1612]/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
