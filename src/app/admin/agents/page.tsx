    ...
    <div className={`flex gap-2.5 group ${isUser ? "flex-row-reverse" : ""}`}>
      ...
      <div className={`max-w-[85%] flex flex-col gap-1 ${isUser ? "items-end" : "items-start"}`}>
        <div className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed font-hanken ${
          isUser ? "bg-[#FF2DA0] text-white rounded-tr-sm" : "bg-[#101014]/[0.055] border border-[#101014]/8 text-[#101014]/80 rounded-tl-sm"
        }`}>
        ...
        <div className={`flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity ${isUser ? "flex-row-reverse" : ""}`}>
          <span className="font-mono text-[9px] text-[#101014]/18">{fmt(msg.ts)}</span>
          <button onClick={copy} className="font-mono text-[9px] text-[#101014]/22 hover:text-[#101014]/60 flex items-center gap-1 transition-colors">
  ...
  return (
    <button onClick={run} disabled={status === "running"}
      className={`flex items-center gap-2.5 w-full px-3.5 py-3 rounded-xl border text-left transition-all group ${
        ...
        "border-[#101014]/8 bg-[#101014]/[0.02] hover:border-[#101014]/16 hover:bg-[#101014]/[0.04]"
      } disabled:opacity-50`}>
      <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 text-[#101014]/30 group-hover:text-[#101014]/60 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className={`font-hanken text-sm ${status === "done" ? "text-[#34d399]" : status === "error" ? "text-[#f87171]" : "text-[#101014]/70"}`}>
          {status === "running" ? "En cours…" : status === "done" ? "Terminé ✓" : status === "error" ? "Erreur" : task.label}
        </p>
        <p className="font-mono text-[9px] text-[#101014]/22 truncate">{task.description}</p>
      </div>
      {status === "idle" && <Play size={11} className="text-[#101014]/15 group-hover:text-[#101014]/40 shrink-0 transition-colors" />}
    </button>
  );
}
...
function AgentPanel({ agent }: { agent: AgentConfig }) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Agent identity */}
      <div className="flex items-center gap-3 pb-4 mb-4 border-b border-[#101014]/6">
        ...
        <div>
          ...
          <p className="font-mono text-[10px] text-[#101014]/30">{agent.jobTitle}</p>
        </div>
      </div>

      {/* Tasks */}
      <div className="mb-4">
        <p className="font-mono text-[9px] uppercase tracking-widest text-[#101014]/22 mb-2.5 flex items-center gap-1.5">
      ...
      {/* Chat */}
      <div className="flex-1 flex flex-col min-h-0 bg-[#101014]/[0.015] rounded-xl border border-[#101014]/6 overflow-hidden">
        <div className="px-3 py-2 border-b border-[#101014]/5 flex items-center justify-between">
          <p className="font-mono text-[9px] text-[#101014]/25 uppercase tracking-widest flex items-center gap-1.5">
            <Bot size={8} /> Instructions directes
          </p>
          {messages.length > 0 && (
            <button onClick={() => setMessages([])} className="font-mono text-[9px] text-[#101014]/18 hover:text-[#f87171]/60 flex items-center gap-1 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
          {messages.length === 0 && (
            <div className="text-center py-6">
              <p className="font-hanken text-xs text-[#101014]/20 leading-relaxed">
                Donne une instruction à {agent.name}<br />
                <span className="text-[#101014]/12">ou lance une t-che ci-dessus</span>
              </p>
              <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                {agent.starters.map((s, i) => (
                  <button key={i} onClick={() => send(s)}
                    className="font-mono text-[9px] px-2.5 py-1.5 rounded-lg border border-[#101014]/8 text-[#101014]/30 hover:text-[#101014]/60 hover:border-[#101014]/16 transition-all">
                    {s}
                  </button>
                ))}
              </div>
          ...
          {loading && (
            <div className="flex gap-2.5">
              ...
              <div className="px-3.5 py-2.5 rounded-2xl rounded-tl-sm bg-[#101014]/[0.055] border border-[#101014]/8 flex items-center gap-1.5">
                {[0,1,2].map(i => (
        ...
        <div className="px-3 py-2.5 border-t border-[#101014]/5 flex gap-2 items-end">
          <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
            onKeyDown={onKeyDown} rows={1} disabled={loading}
            placeholder={`Instruction pour ${agent.name}…`}
            className="flex-1 bg-transparent font-hanken text-sm text-[#101014] placeholder-[#101014]/18 focus:outline-none resize-none disabled:opacity-40 max-h-20 overflow-y-auto" />
  ...
  return (
    <div className="flex gap-5 h-[calc(100vh-7rem)]">
      ...
      <div className="w-52 flex-shrink-0 flex flex-col gap-2">
        <div className="mb-1">
          <h1 className="font-fraunces text-xl text-[#101014]">Équipe IA</h1>
          <p className="font-mono text-[10px] text-[#101014]/28 mt-0.5">Tes salarié·es virtuels</p>
        </div>
          ...
          return (
            <button key={a.id} onClick={() => setActiveId(a.id)}
              className={`w-full text-left p-3.5 rounded-xl border transition-all ${
                active ? "" : "border-[#101014]/6 hover:border-[#101014]/12 hover:bg-[#101014]/[0.02]"
              }`}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-hanken text-sm font-medium truncate" style={{ color: active ? a.color : "#101014" }}>
                    {a.name}
                  </p>
                  <p className="font-mono text-[8.5px] text-[#101014]/25 truncate leading-tight mt-0.5">{a.jobTitle}</p>
                </div>
        ...
        {/* Team note */}
        <div className="mt-auto p-3 rounded-xl bg-[#101014]/[0.025] border border-[#101014]/6">
          <p className="font-mono text-[9px] text-[#101014]/22 leading-relaxed">
