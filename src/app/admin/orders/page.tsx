    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={fetchOrders}
          className="flex items-center gap-2 px-3 py-2 rounded-lg border border-white/[0.14] text-[#1A1612]/40 hover:text-[#1A1612] transition-colors text-sm">
          <RefreshCw size={13} />
        </button>
      </div>
        ...
        <>
          <div className="rounded-xl border border-white/[0.13] overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {orders.map(order => {
                  const statusUi = STATUS_CONFIG[order.status] ?? { label: order.status, color: "text-[#1A1612]/40 bg-white/[0.09] border-white/[0.14]" };
                  return (
