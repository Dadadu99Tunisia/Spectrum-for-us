...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  delivered: { label: "Livré",      color: "text-green-400 bg-green-400/10 border-green-400/20" },
    ...
    <div className="space-y-6">
      {toast && (
        <div className={`fixed top-16 right-6 z-50 px-4 py-2 rounded-lg font-hanken text-sm shadow-xl ${
          toast.startsWith("Erreur") ? "bg-red-500 text-white" : "bg-[#E0337E] text-white"
        }`}>{toast}</div>
      )}

      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-fraunces text-2xl text-[#F3EADB]">Commandes</h1>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="flex gap-1 p-1 bg-[#F3EADB]/4 rounded-lg flex-wrap">
          {STATUS_FILTERS.map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-1.5 rounded-md font-mono text-[10px] transition-all ${
                statusFilter === s ? "bg-[#E0337E] text-white" : "text-[#F3EADB]/40 hover:text-[#F3EADB]"
              }`}>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <ShoppingCart size={40} className="mx-auto mb-3 text-[#F3EADB]/10" />
          <p className="font-hanken text-[#F3EADB]/30">Aucune commande</p>
        </div>
      ) : (
        <>
          <div className="rounded-xl border border-[#F3EADB]/8 overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {orders.map(order => {
                  const statusUi = STATUS_CONFIG[order.status] ?? { label: order.status, color: "text-[#F3EADB]/40 bg-[#F3EADB]/5 border-[#F3EADB]/10" };
                  return (
                    <tr key={order.id} className="border-b border-[#F3EADB]/4 hover:bg-[#F3EADB]/2 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span className="font-fraunces text-sm text-[#F3EADB]">
                        ...
                        <div className="flex items-center gap-1">
                          {order.status === "paid" && !order.refund_status && (
                            <button
                              ...
                              title="Rembourser"
                              className="p-1.5 rounded-lg text-[#F3EADB]/25 hover:text-[#E0901E] transition-colors disabled:opacity-40">
                              <RotateCcw size={12} />
                            </button>
                          )}
