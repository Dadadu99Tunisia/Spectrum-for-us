...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  pending:   { label: "À contacter", color: "text-[#F3EADB]/40 bg-[#F3EADB]/5 border-[#F3EADB]/10",  icon: Clock },
    ...
    <div className="space-y-6">
        ...
        {STATUSES.map(s => {
          ...
          return (
            <button key={s} onClick={() => setStatusFilter(statusFilter === s ? "" : s)}
              className={`p-3 rounded-xl border text-left transition-all ${statusFilter === s ? "border-[#E0337E]/40 bg-[#E0337E]/5" : "border-[#F3EADB]/8 hover:border-[#F3EADB]/15"}`}>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher…"
          className="w-full max-w-sm pl-9 pr-4 py-2 bg-[#F3EADB]/5 border border-[#F3EADB]/10 rounded-lg font-hanken text-sm text-[#F3EADB] placeholder-[#F3EADB]/25 focus:outline-none focus:border-[#E0337E]/50 transition-colors" />
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {items.map(item => {
                ...
                const Icon = cfg.icon;
                return (
                  <tr key={item.id} className="border-b border-[#F3EADB]/4 hover:bg-[#F3EADB]/2 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <select value={item.outreach_status} onChange={e => updateStatus(item.id, e.target.value)}
                        className="bg-[#0e061a] border border-[#F3EADB]/10 rounded px-2 py-1 font-mono text-[9px] text-[#F3EADB]/50 focus:outline-none focus:border-[#E0337E]/40 transition-colors">
