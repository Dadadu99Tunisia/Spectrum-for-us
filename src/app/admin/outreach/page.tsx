...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  pending:   { label: "À contacter", color: "text-[#1A1612]/40 bg-[#1A1612]/[0.09] border-[#1A1612]/[0.14]",  icon: Clock },
    ...
    <div className="space-y-6">
        ...
        {STATUSES.map(s => {
          ...
          return (
            <button key={s} onClick={() => setStatusFilter(statusFilter === s ? "" : s)}
              className={`p-3 rounded-xl border text-left transition-all ${statusFilter === s ? "border-[#FF3D7F]/40 bg-[#FF3D7F]/5" : "border-[#1A1612]/[0.13] hover:border-[#1A1612]/15"}`}>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher…"
          className="w-full max-w-sm pl-9 pr-4 py-2 bg-[#1A1612]/[0.09] border border-[#1A1612]/[0.14] rounded-lg font-hanken text-sm text-[#1A1612] placeholder-[#1A1612]/25 focus:outline-none focus:border-[#a78bfa]/50 transition-colors" />
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {items.map(item => {
                ...
                const Icon = cfg.icon;
                return (
                  <tr key={item.id} className="border-b border-[#1A1612]/[0.05] hover:bg-[#1A1612]/[0.07] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <select value={item.outreach_status} onChange={e => updateStatus(item.id, e.target.value)}
                        className="bg-[#0e061a] border border-[#1A1612]/[0.14] rounded px-2 py-1 font-mono text-[9px] text-[#1A1612]/50 focus:outline-none focus:border-[#FF3D7F]/40 transition-colors">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="bg-[#0e061a] border border-[#1A1612]/[0.14] rounded-2xl p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input value={(form as Record<string, string>)[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))}
                  ...
                  className="w-full bg-[#1A1612]/[0.09] border border-[#1A1612]/[0.14] rounded-lg px-3 py-2 font-hanken text-sm text-[#1A1612] placeholder-[#1A1612]/20 focus:outline-none focus:border-[#a78bfa]/50 transition-colors" />
              </div>
            ))}
