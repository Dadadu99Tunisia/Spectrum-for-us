                    ...
                    <div className="flex gap-2">
                      {Object.entries(themeColors).map(([k, c]) => (
                        <div key={k} className="flex flex-col items-center gap-1">
                          <div className="w-8 h-8 rounded-lg border border-[#1A1612]/10" style={{ background: c }} />
        ...
        <div key={p.id} className="flex items-center gap-4 p-4 rounded-2xl border border-[#1A1612]/8 hover:border-[#1A1612]/15 transition-all">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className={`font-mono text-[8px] px-1.5 py-0.5 rounded border ${p.is_published ? "text-green-600 border-green-400/20" : "text-[#1A1612]/25 border-[#1A1612]/10"}`}>
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => del(p.id)} className="p-2 rounded-lg text-[#1A1612]/20 hover:text-red-600 transition-colors"><Trash2 size={12} /></button>
          </div>
        </div>
      ))}
    </div>
      ...
      <div className="rounded-2xl border border-[#1A1612]/8 overflow-hidden">
        {filtered.map((item, idx) => (
          <div key={item.id} className={`flex items-center gap-3 px-4 py-3 border-b border-[#1A1612]/5 last:border-0 ${!item.is_active ? "opacity-40" : ""}`}>
            ...
            <button onClick={() => toggleActive(item)} className={`p-1.5 rounded-lg transition-colors ${item.is_active ? "text-[#1A1612]/30 hover:text-[#1A1612]" : "text-[#1A1612]/15 hover:text-green-600"}`}>
              {item.is_active ? <EyeOff size={12} /> : <Eye size={12} />}
            </button>
            <button onClick={() => del(item.id)} className="p-1.5 rounded-lg text-[#1A1612]/20 hover:text-red-600 transition-colors"><Trash2 size={12} /></button>
          </div>
        ))}
      ...
      {items.length === 0 && !adding ? (
        ...
        <div key={t.id} className={`flex items-start gap-4 p-4 rounded-2xl border transition-all ${!t.is_active ? "opacity-40" : "border-[#1A1612]/8 hover:border-[#1A1612]/15"}`}>
          ...
          <div className="flex gap-1 flex-shrink-0">
            ...
            <button onClick={() => del(t.id)} className="p-1.5 rounded-lg text-[#1A1612]/20 hover:text-red-600 transition-colors"><Trash2 size={12} /></button>
          </div>
        </div>
      ))}
    </div>
