...
function GrowthCard({ label, value, trend, sub, color, icon: Icon, href, urgent }: {
  ...
  const trendColor = trend === undefined ? "" : trend > 0 ? "text-emerald-400" : trend < 0 ? "text-red-400" : "text-[#1A1612]/30";

  const content = (
    <div className={`relative p-4 rounded-2xl border transition-all group cursor-default overflow-hidden
      ${urgent ? "border-[#FF3D7F]/30 bg-[#FF3D7F]/5" : "border-white/[0.13] bg-white/[0.07] hover:bg-white/[0.055]"}`}>
      {/* Gradient glow */}
...
export default function AdminDashboard() {
  ...
  const ALL_TOOLS = [
    { section: "Opérations", color: "#E0901E", tools: [
    ...
    { section: "Communauté", color: "#6D2DB5", tools: [
    ...
    { section: "Business", color: "#1C9C95", tools: [
    ...
    { section: "Contenu", color: "#CF3F7C", tools: [
    ...
    <div className="space-y-7 max-w-[1440px]">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleRefresh} disabled={refreshing}
          className="flex items-center gap-2 px-4 py-2 rounded-xl border border-white/[0.13] bg-white/[0.07] font-hanken text-sm text-[#1A1612]/50 hover:text-[#1A1612] hover:bg-white/[0.06] transition-all disabled:opacity-40">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            ...
            <GrowthCard
              ...
              trend={g?.membersTrend} sub="ce mois" color="#CF3F7C" icon={Users}
              href="/admin/users"
            />
            <GrowthCard
              label="Total membres" value={g?.totalMembers ?? 0}
              sub="depuis le lancement" color="#6D2DB5" icon={Users}
              href="/admin/users"
            />
            <GrowthCard
              label="Nouveaux vendeurs" value={g?.newVendorsMonth ?? 0}
              trend={g?.vendorsTrend} sub="ce mois" color="#E0901E" icon={Store}
              href="/admin/vendors"
            />
            <GrowthCard
              label="Demandes rejoindre" value={g?.newJoinRequestsWeek ?? 0}
              sub="cette semaine" color="#1C9C95" icon={UserPlus}
          ...
          {chartMembers.some(p => p.count > 0) ? (
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={chartMembers} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(26,22,18,.06)" />
                <XAxis dataKey="date" tick={{ fill: "rgba(26,22,18,.3)", fontSize: 9 }} tickLine={false} axisLine={false} interval={6} />
                <YAxis tick={{ fill: "rgba(26,22,18,.3)", fontSize: 9 }} tickLine={false} axisLine={false} allowDecimals={false} />
                <Tooltip
                  ...
                  labelStyle={{ color: "rgba(26,22,18,.5)" }} />
              ...
              <AreaChart data={chart} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                <defs>
                  <linearGradient id="rev" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%"  stopColor="#6D2DB5" stopOpacity={0.4} />
                    <stop offset="95%" stopColor="#6D2DB5" stopOpacity={0} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(26,22,18,.06)" />
                <XAxis dataKey="date" tick={{ fill: "rgba(26,22,18,.3)", fontSize: 9 }} tickLine={false} axisLine={false} interval={4} />
                <YAxis tick={{ fill: "rgba(26,22,18,.3)", fontSize: 9 }} tickLine={false} axisLine={false}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        {/* Quick actions */}
        <Card>
          <SectionTitle accent="#E0901E">Actions prioritaires</SectionTitle>
            ...
            <div className="space-y-1">
              {recentOrders.map(o => (
                <div key={String(o.id)}
                  ...
                  <span className={`font-mono text-[9px] px-2 py-0.5 rounded-full border ${STATUS_PILL[String(o.status)] ?? "bg-white/[0.06] text-[#1A1612]/40 border-white/[0.13]"}`}>
