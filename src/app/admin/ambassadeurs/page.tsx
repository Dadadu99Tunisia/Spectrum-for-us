...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  pending:   { label: "Prospect",   color: "text-[#101014]/40 bg-[#101014]/5 border-[#101014]/10" },
  contacted: { label: "Contacté",   color: "text-[#FFD400] bg-[#FFD400]/10 border-[#FFD400]/20" },
  replied:   { label: "A répondu",  color: "text-[#7A2BF0] bg-[#7A2BF0]/10 border-[#7A2BF0]/20" },
  converted: { label: "Ambassadeur ✓", color: "text-[#FF2DA0] bg-[#FF2DA0]/10 border-[#FF2DA0]/20" },
  rejected:  { label: "Refusé",     color: "text-red-600 bg-red-400/10 border-red-400/20" },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-fraunces text-2xl text-[#101014]">Ambassadeur·rices</h1>
          <p className="font-hanken text-sm text-[#101014]/40 mt-0.5">{total} profil{total !== 1 ? "s" : ""}</p>
        </div>
      ...
      {/* Filter tabs */}
      <div className="flex gap-1 p-1 bg-[#101014]/4 rounded-xl w-fit flex-wrap">
        {[["converted","Ambassadeurs actifs"],["pending","Prospects"],["contacted","Contactés"],["","Tous"]].map(([v,l]) => (
          <button key={v} onClick={() => setStatusFilter(v)}
            className={`px-3 py-1.5 rounded-lg font-mono text-[10px] transition-all ${statusFilter === v ? "bg-[#FF2DA0] text-white" : "text-[#101014]/40 hover:text-[#101014]"}`}>{l}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20"><Star size={40} className="mx-auto mb-3 text-[#101014]/10" /><p className="font-hanken text-[#101014]/30">Aucun ambassadeur·rice</p></div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {items.map(a => {
            const cfg = STATUS_CONFIG[a.outreach_status] ?? STATUS_CONFIG.pending;
            return (
              <div key={a.id} className="p-5 rounded-2xl border border-[#101014]/8 hover:border-[#101014]/15 transition-all space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#FF2DA0]/10 border border-[#FF2DA0]/20 flex items-center justify-center flex-shrink-0">
                    <span className="font-fraunces text-base text-[#FF2DA0]">{a.name[0].toUpperCase()}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-hanken text-sm text-[#101014] font-medium">{a.name}</p>
                    {a.category && <p className="font-mono text-[9px] text-[#101014]/35 uppercase mt-0.5">{a.category}</p>}
                  </div>
                  <span className={`font-mono text-[8px] px-2 py-1 rounded-full border flex-shrink-0 ${cfg.color}`}>{cfg.label}</span>
                </div>
                {a.followers_count && (
                  <p className="font-mono text-sm text-[#101014]/60">
                    <span className="text-[#FF2DA0] font-medium">{(a.followers_count / 1000).toFixed(1)}k</span>
                    <span className="text-[#101014]/30 text-[10px] ml-1">abonnés</span>
                  </p>
                )}
                {a.notes && <p className="font-hanken text-xs text-[#101014]/40 line-clamp-2">{a.notes}</p>}
                <div className="flex items-center justify-between pt-2 border-t border-[#101014]/6">
                  <div className="flex gap-2">
                    {a.email && <a href={`mailto:${a.email}`} className="text-[#101014]/25 hover:text-[#FF2DA0] transition-colors"><Mail size={13} /></a>}
                    {a.instagram_handle && (
                      <a href={`https://instagram.com/${a.instagram_handle}`} target="_blank" rel="noreferrer"
                        className="text-[#101014]/25 hover:text-[#FF2DA0] transition-colors"><Link2 size={13} /></a>
                    )}
                    {a.profile_url && (
                      <a href={a.profile_url} target="_blank" rel="noreferrer"
                        className="text-[#101014]/25 hover:text-[#101014] transition-colors"><Globe size={13} /></a>
                    )}
                  </div>
                  <select value={a.outreach_status} onChange={e => updateStatus(a.id, e.target.value)}
                    className="bg-transparent border border-[#101014]/10 rounded px-2 py-1 font-mono text-[9px] text-[#101014]/40 focus:outline-none focus:border-[#FF2DA0]/30 transition-colors">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && setShowForm(false)}>
          <div className="bg-[#0e061a] border border-[#101014]/10 rounded-2xl p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-fraunces text-lg text-[#101014]">Nouvel·le ambassadeur·rice</h2>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input value={(form as Record<string, string>)[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))}
                  ...
                  className="w-full bg-[#101014]/5 border border-[#101014]/10 rounded-lg px-3 py-2 font-hanken text-sm text-[#101014] placeholder-[#101014]/20 focus:outline-none focus:border-[#FF2DA0]/50 transition-colors" />
              </div>
            ))}
            <button onClick={addAmbassador} disabled={!form.name || saving}
              className="w-full py-2.5 rounded-xl bg-[#FF2DA0] text-white font-hanken text-sm hover:bg-[#FF2DA0]/90 disabled:opacity-40 transition-colors">
