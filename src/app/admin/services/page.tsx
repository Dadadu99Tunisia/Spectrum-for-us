...
const STATUS_CFG: Record<string, { label: string; color: string }> = {
  ...
  inactive: { label: "Inactif",    color: "text-[#101014]/30 bg-[#101014]/5 border-[#101014]/10" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-fraunces text-2xl text-[#101014]">Services & Ateliers</h1>
          <p className="font-hanken text-sm text-[#101014]/40 mt-0.5">{total} service{total !== 1 ? "s" : ""}</p>
        </div>
        <button onClick={fetch_} className="flex items-center gap-2 px-3 py-2 rounded-lg border border-[#101014]/10 text-[#101014]/40 hover:text-[#101014] transition-colors">
          <RefreshCw size={13} />
        </button>
      </div>
        ...
        <div className="flex gap-1 p-1 bg-[#101014]/4 rounded-lg">
          {[["","Tous"],["active","Actifs"],["pending","En attente"],["inactive","Inactifs"],["rejected","Rejetés"]].map(([v,l]) => (
            <button key={v} onClick={() => setStatusFilter(v)}
              className={`px-3 py-1.5 rounded-md font-mono text-[10px] transition-all ${statusFilter === v ? "bg-[#FF2DA0] text-white" : "text-[#101014]/40 hover:text-[#101014]"}`}>{l}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20"><Sparkles size={40} className="mx-auto mb-3 text-[#101014]/10" /><p className="font-hanken text-[#101014]/30">Aucun service</p></div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {items.map(w => {
            ...
            return (
              <div key={w.id} className="p-5 rounded-2xl border border-[#101014]/8 hover:border-[#101014]/15 transition-all space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="w-10 h-10 rounded-xl bg-[#7A2BF0]/10 border border-[#7A2BF0]/20 flex items-center justify-center flex-shrink-0">
                    <Sparkles size={15} className="text-[#7A2BF0]" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-hanken text-sm text-[#101014] font-medium truncate">{displayTitle}</p>
                    {w.category && <p className="font-mono text-[9px] text-[#101014]/35 uppercase mt-0.5">{w.category}</p>}
                  </div>
                  <button onClick={() => toggleFeatured(w.id, w.is_featured)}
                    className={`flex-shrink-0 p-1 rounded transition-colors ${w.is_featured ? "text-[#FFD400]" : "text-[#101014]/15 hover:text-[#FFD400]"}`}>
                ...
                {w.price !== null && (
                  <p className="font-fraunces text-base text-[#101014]">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-[#101014]/6">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => toggleActive(w.id, w.is_active)}
                      className={`p-1.5 rounded-lg border border-transparent transition-colors ${w.is_active ? "text-green-600/60 hover:text-red-600 hover:border-red-400/20" : "text-[#101014]/20 hover:text-green-600 hover:border-green-400/20"}`}
