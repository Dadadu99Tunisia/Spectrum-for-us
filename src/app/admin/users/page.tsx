...
const ROLE_COLOR: Record<string, string> = {
  ...
  vendor:      "text-[#1A1612]/60 bg-white/[0.09] border-white/[0.14]",
  buyer:       "text-[#1A1612]/30 bg-[#1A1612]/3 border-white/[0.13]",
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
          className="bg-white/[0.09] border border-white/[0.14] rounded-lg px-3 py-2 font-mono text-xs text-[#1A1612]/60 focus:outline-none focus:border-[#a78bfa]/50 transition-colors">
                      ...
                      {editRole?.id === u.id ? (
                        <div className="flex items-center gap-1">
                          <select defaultValue={u.role}
                            ...
                            className="bg-[#FBF9F5] border border-[#FF3D7F]/40 rounded px-2 py-1 font-mono text-[10px] text-[#1A1612] focus:outline-none text-xs">
                        ...
                        <button onClick={() => setEditRole({ id: u.id, current: u.role })}
                          className={`flex items-center gap-1 font-mono text-[9px] px-2 py-1 rounded-full border transition-colors hover:opacity-80 ${ROLE_COLOR[u.role] ?? "text-[#1A1612]/30 bg-white/[0.09] border-white/[0.14]"}`}>
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-colors disabled:opacity-40 ${
                          u.is_suspended
                            ...
                            : "text-[#1A1612]/25 hover:text-red-400 border border-white/[0.13] hover:border-red-500/20"
                        }`}>
