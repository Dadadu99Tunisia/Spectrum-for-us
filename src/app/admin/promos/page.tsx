...
export default function AdminPromosPage() {
  ...
  const load = useCallback(async () => {
    const { data } = await createClient().from("discount_codes").select("*").is("shop_id", null).order("created_at", { ascending: false });
  ...
  const create = async () => {
    ...
    const { error } = await createClient().from("discount_codes").insert({
      shop_id: null, code: form.code.trim().toUpperCase(), kind: form.kind, value: Number(form.value),
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-fraunces text-2xl text-[#101014] flex items-center gap-2"><Tag size={20} /> Codes promo plateforme</h1>
          <p className="font-hanken text-sm text-[#101014]/40 mt-0.5">Campagnes globales (toutes boutiques). La remise est absorbée par la plateforme.</p>
        </div>
      ...
      {showForm && (
        <div className="rounded-2xl border border-[#FF2DA0]/30 bg-[#FF2DA0]/[0.03] p-4 space-y-3 max-w-2xl">
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block font-mono text-[10px] text-[#101014]/40 mb-1">Code *</label>
              <input value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="SPECTRUM10" className={I + " w-full uppercase"} /></div>
            <div><label className="block font-mono text-[10px] text-[#101014]/40 mb-1">Type</label>
              <select value={form.kind} onChange={e => setForm({ ...form, kind: e.target.value as "percent" | "fixed" })} className={I + " w-full"}>
                <option value="percent">Pourcentage (%)</option><option value="fixed">Montant fixe (€)</option></select></div>
          ...
          <button onClick={create} disabled={saving} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#101014] text-white font-hanken text-sm disabled:opacity-40">
            {saving ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} Créer le code</button>
        </div>
      )}
      ...
      {codes === null ? <div className="flex items-center gap-2 text-[#101014]/40 py-6"><Loader2 size={15} className="animate-spin" /> Chargement…</div>
        : codes.length === 0 ? <p className="font-hanken text-sm text-[#101014]/40 py-6">Aucun code plateforme.</p>
        : (
            ...
            return (
              <div key={c.id} className="flex items-center justify-between gap-3 rounded-2xl border border-[#101014]/12 bg-white p-3.5">
                <div>
                  ...
                  <p className="font-mono text-[10px] text-[#101014]/40">{c.min_order ? `min ${c.min_order} € · ` : ""}{c.used_count}{c.max_uses ? `/${c.max_uses}` : ""} utilisé{c.used_count > 1 ? "s" : ""}{c.expires_at ? ` · exp. ${new Date(c.expires_at).toLocaleDateString("fr-FR")}` : ""}{expired && <span className="text-red-500"> · expiré</span>}</p>
                </div>
