  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-fraunces text-2xl text-[#1A1612]">Articles</h1>
          <p className="font-hanken text-sm text-[#1A1612]/40 mt-0.5">{total} article{total !== 1 ? "s" : ""}</p>
        </div>
        ...
        <div className="flex gap-1 p-1 bg-[#1A1612]/4 rounded-lg">
          {[["all","Tous"],["published","Publiés"],["draft","Brouillons"]].map(([v,l]) => (
            <button key={v} onClick={() => setPubFilter(v as typeof pubFilter)}
              className={`px-3 py-1.5 rounded-md font-mono text-[10px] transition-all ${pubFilter === v ? "bg-[#FF3D7F] text-white" : "text-[#1A1612]/40 hover:text-[#1A1612]"}`}>{l}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20"><FileText size={40} className="mx-auto mb-3 text-[#1A1612]/10" /><p className="font-hanken text-[#1A1612]/30">Aucun article</p></div>
      ) : (
        <div className="space-y-2">
          {articles.map(a => (
            <div key={a.id} className="flex items-center gap-4 p-4 rounded-xl border border-[#1A1612]/8 hover:border-[#1A1612]/15 transition-all group">
              {a.cover_url ? (
                <img src={a.cover_url} alt="" className="w-14 h-10 rounded-lg object-cover border border-[#1A1612]/10 flex-shrink-0" />
              ) : (
                <div className="w-14 h-10 rounded-lg bg-[#1A1612]/5 border border-[#1A1612]/10 flex items-center justify-center flex-shrink-0">
                  <FileText size={14} className="text-[#1A1612]/20" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                  <p className="font-hanken text-sm text-[#1A1612] truncate">{a.title_fr || a.title_en || "(Sans titre)"}</p>
                  {a.category && (
                    <span className="font-mono text-[9px] px-1.5 py-0.5 rounded bg-[#6D2DB5]/15 text-[#6D2DB5]/80 uppercase">{a.category}</span>
                  )}
                </div>
                {a.excerpt_fr && <p className="font-hanken text-xs text-[#1A1612]/30 truncate">{a.excerpt_fr}</p>}
                <div className="flex items-center gap-3 mt-1">
                  <span className="font-mono text-[9px] text-[#1A1612]/20">
                    {new Date(a.created_at).toLocaleDateString("fr-FR")}
                  </span>
                  {a.tags?.slice(0,3).map(t => (
                    <span key={t} className="font-mono text-[8px] text-[#1A1612]/20 border border-[#1A1612]/8 px-1.5 rounded">#{t}</span>
                  ))}
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className={`font-mono text-[9px] px-2 py-1 rounded-full border ${
                  a.published
                    ...
                    : "text-[#1A1612]/30 bg-[#1A1612]/5 border-[#1A1612]/10"
                }`}>{a.published ? "Publié" : "Brouillon"}</span>
                <button onClick={() => togglePublish(a.id, a.published)}
                  className="p-1.5 rounded-lg text-[#1A1612]/25 hover:text-[#1A1612] border border-transparent hover:border-[#1A1612]/10 transition-colors"
