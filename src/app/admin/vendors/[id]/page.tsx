...
const KYC_STATUS_UI: Record<string, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  pending:   { label: "En attente",   color: "text-[#101014]/50", bg: "bg-[#101014]/5 border-[#101014]/10",   icon: Clock },
  submitted: { label: "Soumis",       color: "text-[#FFD400]",    bg: "bg-[#FFD400]/10 border-[#FFD400]/20",   icon: AlertCircle },
...
export default function VendorDetailPage() {
  ...
  if (!data) return (
    <div className="text-center py-20">
      <p className="font-hanken text-[#101014]/40">Boutique introuvable</p>
    </div>
  );
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      {/* Toast */}
      {toast && (
        <div className="fixed top-16 right-6 z-50 px-4 py-2 rounded-lg bg-[#FF2DA0] text-white font-hanken text-sm shadow-xl">
          {toast}
        </div>
      )}
      ...
      {/* Shop header */}
      <div className="flex items-start gap-5 p-6 rounded-2xl bg-[#101014]/3 border border-[#101014]/8">
        <div className="w-14 h-14 rounded-xl bg-[#7A2BF0]/20 border border-[#7A2BF0]/30 flex items-center justify-center flex-shrink-0">
          <Store size={22} className="text-[#7A2BF0]" />
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="font-fraunces text-2xl text-[#101014]">{shop.name as string}</h1>
            ...
            <span className={`w-2 h-2 rounded-full ${shop.is_active ? "bg-green-400" : "bg-[#101014]/15"}`} title={shop.is_active ? "Active" : "Inactive"} />
          </div>
          <p className="font-mono text-xs text-[#101014]/30 mt-1">/{shop.slug as string}</p>
          <div className="flex items-center gap-4 mt-3">
            ...
            <span className="flex items-center gap-1 font-mono text-[10px] text-[#101014]/25">
              <Package size={9} /> {product_count} produits
            </span>
          </div>
        </div>
        <a href={`/boutique/${shop.slug}`} target="_blank" rel="noreferrer"
          className="p-2 rounded-lg border border-[#101014]/10 text-[#101014]/30 hover:text-[#101014] transition-colors">
          <ExternalLink size={13} />
        </a>
      </div>

      {/* Gestion boutique */}
      <div className="rounded-2xl border border-[#101014]/8 p-5 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h2 className="font-fraunces text-base text-[#101014]">Gestion de la boutique</h2>
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => patchShop({ is_active: !shop.is_active }, "active")} disabled={!!actionLoading}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border font-mono text-[10px] transition-colors disabled:opacity-40 ${shop.is_active ? "bg-green-500/10 border-green-500/20 text-green-600" : "bg-[#101014]/[0.07] border-[#101014]/[0.14] text-[#101014]/50"}`}>
            ...
            <button onClick={() => patchShop({ is_verified: !shop.is_verified }, "verified")} disabled={!!actionLoading}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border font-mono text-[10px] transition-colors disabled:opacity-40 ${shop.is_verified ? "bg-[#2323C4]/10 border-[#2323C4]/20 text-[#2323C4]" : "bg-[#101014]/[0.07] border-[#101014]/[0.14] text-[#101014]/50"}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {([["name","Nom"],["tagline","Slogan"],["city","Ville"],["country","Pays"],["contact_email","Email contact"]] as const).map(([k, label]) => (
              <div key={k} className={k === "tagline" ? "md:col-span-2" : ""}>
                <label className="block font-mono text-[10px] uppercase tracking-wide text-[#101014]/35 mb-1.5">{label}</label>
                <input value={form[k]} onChange={e => setForm({ ...form, [k]: e.target.value })}
                  className="w-full px-3 py-2 bg-[#101014]/[0.07] border border-[#101014]/[0.14] rounded-lg font-hanken text-sm text-[#101014] focus:outline-none focus:border-[#a78bfa]/50" />
              </div>
            ))}
            <div className="md:col-span-2">
              <label className="block font-mono text-[10px] uppercase tracking-wide text-[#101014]/35 mb-1.5">Description</label>
              <textarea rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                className="w-full px-3 py-2 bg-[#101014]/[0.07] border border-[#101014]/[0.14] rounded-lg font-hanken text-sm text-[#101014] focus:outline-none focus:border-[#a78bfa]/50" />
            </div>
            <div className="md:col-span-2">
              <button onClick={() => patchShop(form, "save")} disabled={!!actionLoading}
                className="flex items-center gap-1.5 px-5 py-2.5 rounded-lg bg-[#FF2DA0] text-white font-hanken font-semibold text-sm hover:brightness-110 disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* KYC Panel */}
        <div className="rounded-2xl border border-[#101014]/8 p-5 space-y-4">
          <h2 className="font-fraunces text-base text-[#101014]">Informations KYC</h2>
          {kyc ? (
            <div className="space-y-3">
              {[
                ...
                <div key={String(label)} className="flex justify-between gap-4">
                  <span className="font-mono text-[10px] text-[#101014]/30 uppercase tracking-wide flex-shrink-0">{String(label)}</span>
                  <span className="font-hanken text-xs text-[#101014]/70 text-right">{String(value)}</span>
                </div>
              ) : null)}
              {Boolean(kyc.instagram_url) && (
                <a href={kyc.instagram_url as string} target="_blank" rel="noreferrer"
                  className="flex items-center gap-2 text-[#FF2DA0] font-hanken text-xs hover:underline">
                  <Link2 size={12} /> Instagram
                </a>
              )}
              {Boolean(kyc.website_url) && (
                <a href={kyc.website_url as string} target="_blank" rel="noreferrer"
                  className="flex items-center gap-2 text-[#2323C4] font-hanken text-xs hover:underline">
                  <Globe size={12} /> Site web
                </a>
              )}
            </div>
          ) : (
            <p className="font-hanken text-sm text-[#101014]/30">KYC non soumis</p>
          )}

          {/* KYC Actions */}
          {kycStatus === "submitted" && (
            <div className="pt-3 border-t border-[#101014]/6 space-y-3">
              <textarea
                value={notes}
                onChange={e => setNotes(e.target.value)}
                placeholder="Notes (visible par le·la vendeur·se si rejeté·e)…"
                rows={2}
                className="w-full bg-[#101014]/5 border border-[#101014]/10 rounded-lg px-3 py-2 text-[#101014] font-hanken text-sm placeholder-[#101014]/25 focus:outline-none focus:border-[#FF2DA0]/50 resize-none" />
        ...
        {/* Recent orders */}
        <div className="rounded-2xl border border-[#101014]/8 p-5 space-y-4">
          <h2 className="font-fraunces text-base text-[#101014]">Dernières commandes</h2>
          {recent_orders.length === 0 ? (
            <p className="font-hanken text-sm text-[#101014]/30">Aucune commande</p>
          ) : (
            <div className="space-y-2">
              {recent_orders.map((o: Record<string, unknown>) => (
                <div key={o.id as string} className="flex items-center justify-between py-2 border-b border-[#101014]/4">
                  <div>
                    <span className="font-mono text-[10px] text-[#101014]/30">#{(o.id as string).slice(0,8).toUpperCase()}</span>
                    <p className="font-mono text-[9px] text-[#101014]/20 mt-0.5">
                      {new Date(o.created_at as string).toLocaleDateString("fr-FR")}
                    </p>
                  </div>
                  <span className="font-fraunces text-sm text-[#101014]">{Number(o.total).toFixed(2)} €</span>
                </div>
              ))}
            </div>
          )}
          <div className="pt-2">
            <h3 className="font-fraunces text-sm text-[#101014] mb-3 flex items-center gap-2">
              <Package size={12} /> Derniers produits
            </h3>
            {recent_products.length === 0 ? (
              <p className="font-hanken text-sm text-[#101014]/30">Aucun produit</p>
            ) : (
              <div className="space-y-2">
                {recent_products.map((p: Record<string, unknown>) => (
                  <div key={p.id as string} className="flex items-center justify-between py-1.5 border-b border-[#101014]/4">
                    <span className="font-hanken text-xs text-[#101014]/70 truncate flex-1">{p.title as string}</span>
                    <span className="font-fraunces text-xs text-[#101014]/50 ml-2">{Number(p.price).toFixed(2)} €</span>
                  </div>
                ))}
              </div>
            )}
