...
export async function sendVendorNewOrder(params: {
  ...
  items: OrderItem[];
  total: number;
  customer?: { name?: string; address?: string; city?: string; zip?: string; country?: string; email?: string; phone?: string } | null;
  ...
  const deliveryBlock = (c || params.shippingMethod) ? `
    <div style="background:#FBF7FB;border-radius:14px;padding:16px 18px;margin:0 0 20px;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        ...
        ${c?.phone ? infoBox("Téléphone", c.phone) : ""}
...
/** Relance critique · le·la créateur·ice a des produits mais n'a pas activé ses paiements (invendable). */
export async function sendActivatePayments(params: { to: string; pseudo: string }) {
  const body = `
  ...
  return getResend().emails.send({
    from: FROM,
    to: params.to,
    subject: "Tes produits sont en ligne mais non achetables 💳",
    html: baseLayout("Active tes paiements · Spectrum For Us", body),
  });
}
