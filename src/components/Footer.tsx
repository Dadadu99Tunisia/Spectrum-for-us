...
export function Footer() {
  return (
    <footer className="border-t border-[#1A1612]/8 pt-16 pb-32 px-6">
      {/* Prism line */}
      <div
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-10 mb-16">
          ...
          <div className="col-span-2 md:col-span-4 lg:col-span-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="font-fraunces text-xl text-[#1A1612] leading-tight">
                Spectrum <span className="text-[#FF3D7F] font-light">For Us</span>
              </div>
            </div>
            <p className="font-hanken text-sm text-[#1A1612]/50 leading-relaxed mb-6 max-w-xs">
            ...
            <div className="flex gap-3">
              <a href="https://www.instagram.com/spectrum.forus/" target="_blank" rel="noopener noreferrer"
                className="p-2 rounded-full border border-[#1A1612]/15 text-[#1A1612]/40 hover:border-[#FF3D7F]/40 hover:text-[#FF3D7F] transition-all duration-200"
          ...
          {Object.entries(LINKS).map(([category, links]) => (
            <div key={category}>
              <h3 className="font-mono text-[10px] tracking-widest uppercase text-[#1A1612]/30 mb-4">
                {category}
              </h3>
              <ul className="space-y-2">
                {links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="font-hanken text-sm text-[#1A1612]/50 hover:text-[#1A1612] transition-colors duration-200"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="border-t border-[#1A1612]/8 pt-6 flex flex-col sm:flex-row gap-3 justify-between items-center">
          <p className="font-mono text-[11px] text-[#1A1612]/25">
            © 2026 Spectrum For Us · Tous droits réservés ·{" "}
            <Link href="/legal/mentions" className="hover:text-[#1A1612]/50 transition-colors">Mentions légales</Link>
          </p>
          <p className="font-mono text-[11px] text-[#1A1612]/25">
            B(u)y us, for us. 🌈
          </p>
