  ...
  return (
    <section ref={ref} className="py-24 px-6 bg-[#2A1540]">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <span className="inline-block font-mono text-[11px] tracking-widest uppercase text-[#FF3D7F] mb-4 px-4 py-1.5 rounded-full border border-[#FF3D7F]/20 bg-[#FF3D7F]/8">
            {t("rejoindre_cta.eyebrow")}
          </span>
          <h2 className="font-fraunces text-4xl md:text-5xl font-light mb-4">
            {t("rejoindre_cta.title")}{" "}
            <span className="italic text-[#1A1612]">{t("rejoindre_cta.title_em")}</span>
          </h2>
          <p className="font-hanken text-[#1A1612]/55 max-w-xl mx-auto leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-12">
          {PROFILES.map((p, i) => (
            <div
              key={p.title}
              className="rounded-2xl border border-[#1A1612]/8 bg-[#1A1612]/[0.025] p-5 hover:border-[#FF3D7F]/25 hover:bg-[#FF3D7F]/5 transition-all duration-300"
              style={{
                opacity: inView ? 1 : 0,
            ...
            >
              ...
              <div className="font-bricolage font-semibold text-[#1A1612] text-base mb-1">{p.title}</div>
              <div className="font-hanken text-xs text-[#1A1612]/45 leading-relaxed">{p.desc}</div>
            </div>
          ))}
        </div>
        ...
        >
          <Link
            href="/rejoindre"
            className="inline-flex items-center gap-2 px-10 py-4 rounded-2xl bg-[#FF3D7F] text-white font-hanken font-semibold text-base hover:bg-[#FF3D7F]/85 hover:scale-[1.02] transition-all duration-200 shadow-xl shadow-[#FF3D7F]/20"
          >
            {t("rejoindre_cta.cta")} <ArrowRight size={16} />
          </Link>
          <p className="font-hanken text-xs text-[#1A1612]/30 mt-4">
