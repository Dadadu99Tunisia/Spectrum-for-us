    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-1.5">
        ...
        <div className="flex items-start gap-2">
          <h3 className="font-bricolage font-bold text-[#1A1612] text-sm leading-snug flex-1 min-w-0">
            {org.name}
          </h3>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="flex items-center gap-1 font-mono text-[10px] text-[#1A1612]/40">
            <MapPin size={9} style={{ color: org.accent }} />
            {codeToFlag(org.countryCode)} {org.city}, {org.country}
          </span>
          {org.founded && (
            <span className="flex items-center gap-0.5 font-mono text-[9px] text-[#1A1612]/20">
        ...
        {/* Description */}
        <p className="font-hanken text-[11px] text-[#1A1612]/40 leading-relaxed line-clamp-2">
          {org.description}
        </p>
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6">
      ...
      <div className="relative w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col max-h-[90vh]"
        ...
        <div className="relative shrink-0 px-6 pt-7 pb-6 overflow-hidden"
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="font-bricolage font-bold text-[#1A1612] text-xl leading-tight">{org.name}</h2>
              <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                <span className="flex items-center gap-1 font-mono text-xs text-[#1A1612]/40">
                  <MapPin size={10} />{org.city}, {org.country}
                </span>
                {org.founded && (
                  <span className="font-mono text-[10px] text-[#1A1612]/25">· Fondée en {org.founded}</span>
                )}
              </div>
            ...
            <button onClick={onClose} aria-label="Fermer"
              className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-[#1A1612]/30 hover:text-[#1A1612] hover:bg-[#1A1612]/8 transition-all">
              <X size={15} />
            </button>
          </div>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 pb-8 space-y-5">
          <p className="font-hanken text-sm text-[#1A1612]/65 leading-relaxed">
            {org.description}
          </p>
          ...
          <div className="space-y-2.5">
            {org.phone && (
              <a href={`tel:${org.phone}`}
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-[9px] text-[#1A1612]/25 uppercase tracking-widest mb-0.5">Téléphone</p>
                  <p className="font-hanken text-sm text-[#1A1612]/80 font-medium">{org.phone}</p>
                </div>
            ...
            {org.email && (
              <a href={`mailto:${org.email}`}
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-[9px] text-[#1A1612]/25 uppercase tracking-widest mb-0.5">E-mail</p>
                  <p className="font-hanken text-sm text-[#1A1612]/80 font-medium truncate">{org.email}</p>
                </div>
                <ExternalLink size={11} className="opacity-0 group-hover:opacity-50 transition-opacity shrink-0" style={{ color: org.accent }} />
              </a>
            )}
            {org.website && (
              <a href={org.website} target="_blank" rel="noopener noreferrer"
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-[9px] text-[#1A1612]/25 uppercase tracking-widest mb-0.5">Site web</p>
                  <p className="font-hanken text-sm text-[#1A1612]/80 font-medium truncate">{domain(org.website)}</p>
                </div>
...
function FilterSection({ title, children, defaultOpen = true }: { title: string; children: React.ReactNode; defaultOpen?: boolean }) {
  ...
  return (
    <div className="border-b border-[#1A1612]/6 pb-4 mb-4">
      <button onClick={() => setOpen(v => !v)}
        className="flex items-center justify-between w-full mb-3 group">
        <span className="font-mono text-[10px] uppercase tracking-widest text-[#1A1612]/35 group-hover:text-[#1A1612]/60 transition-colors">{title}</span>
        <ChevronDown size={12} className="text-[#1A1612]/20 transition-transform duration-200"
          ...
          <div className="p-5">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <SlidersHorizontal size={14} className="text-[#FF3D7F]" />
                <span className="font-bricolage font-bold text-[#1A1612] text-sm">Filtres</span>
                {activeFilters > 0 && (
                  <span className="px-1.5 py-0.5 rounded-full bg-[#FF3D7F] text-white font-mono text-[9px] tabular-nums">
                    {activeFilters}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2">
                {activeFilters > 0 && (
                  <button onClick={resetAll} className="font-mono text-[10px] text-[#FF3D7F]/70 hover:text-[#FF3D7F] transition-colors">
                    Tout effacer
                  </button>
                )}
                <button onClick={() => setShowFilters(false)} className="md:hidden text-[#1A1612]/30 hover:text-[#1A1612]">
                  <X size={16} />
                </button>
              </div>
            </div>
            ...
            <div className="relative mb-5">
              ...
              {search && (
                <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#1A1612]/30 hover:text-[#FF3D7F]">
                  <X size={12} />
                </button>
              )}
            </div>
                ...
                {CATEGORIES.map(cat => {
                  ...
                  return (
                    <button key={cat.value} onClick={() => toggleCat(cat.value)}
                      ...
                      style={{
                        background: active ? `${cat.value === "association" ? "#FF3D7F" : "#1C9C95"}15` : "transparent",
                        border: `1px solid ${active ? (cat.value === "association" ? "#FF3D7F" : "#1C9C95") + "40" : "transparent"}`,
                      }}>
                      <span className="w-5 h-5 rounded-md flex items-center justify-center shrink-0 text-sm"
                        style={{ background: active ? "#1C9C9520" : "rgba(26,22,18,0.06)" }}>
                        {cat.emoji}
                      </span>
                      <span className="flex-1 font-hanken text-xs" style={{ color: active ? "#1A1612" : "rgba(26,22,18,0.50)" }}>
                        {cat.label}
                      </span>
                      <span className="font-mono text-[9px] tabular-nums" style={{ color: active ? "#1C9C95" : "rgba(26,22,18,0.20)" }}>
                        {count}
                      </span>
                      {active && (
              ...
              <div className="space-y-1">
                {visibleCountries.map(c => {
                  ...
                  return (
                    <button key={c} onClick={() => toggleCountry(c)}
                      ...
                      style={{
                        background: active ? "#FF3D7F12" : "transparent",
                        border: `1px solid ${active ? "#FF3D7F40" : "transparent"}`,
                      }}>
                      <span className="text-base leading-none w-6 text-center shrink-0">{codeToFlag(code)}</span>
                      <span className="flex-1 font-hanken text-xs truncate" style={{ color: active ? "#1A1612" : "rgba(26,22,18,0.50)" }}>
                        {c}
                      </span>
                      <span className="font-mono text-[9px] tabular-nums shrink-0" style={{ color: active ? "#FF3D7F" : "rgba(26,22,18,0.20)" }}>
                        {count}
                      </span>
                    </button>
                  );
                ...
                {allCountries.length > 12 && (
                  <button onClick={() => setShowMoreCountries(v => !v)}
                    className="w-full text-center font-mono text-[10px] text-[#1A1612]/25 hover:text-[#1A1612]/60 transition-colors py-1.5">
            ...
            <button onClick={pickRandom}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-[#1A1612]/10 font-hanken text-xs text-[#1A1612]/40 hover:text-[#E0901E] hover:border-[#E0901E]/40 transition-all">
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="sticky top-0 z-10 bg-[#FBF9F5]/95 backdrop-blur-md border-b border-[#1A1612]/8 px-5 py-3 flex items-center gap-3 shrink-0">
            ...
            <button onClick={() => setShowFilters(v => !v)}
              ...
              {activeFilters > 0 && <span className="text-[#FF3D7F]">{activeFilters}</span>}
            </button>

            {/* Result count */}
            <div className="flex items-center gap-1.5 min-w-0">
              <span className="font-bricolage font-bold text-[#1A1612] text-sm tabular-nums">{filtered.length}</span>
              <span className="font-hanken text-xs text-[#1A1612]/35 truncate">
            ...
            <div className="flex items-center gap-1.5 flex-1 min-w-0 overflow-x-auto scrollbar-none">
              {search && (
                <span className="flex items-center gap-1 px-2 py-1 rounded-full bg-[#FF3D7F]/15 border border-[#FF3D7F]/30 font-mono text-[10px] text-[#FF3D7F] shrink-0">
                  "{search}"
            ...
            {/* Sort */}
            <div className="flex items-center gap-1 shrink-0 border border-[#1A1612]/10 rounded-xl overflow-hidden">
              {([["country", "Pays"], ["name", "A–Z"], ["founded", "Ancienneté"]] as [SortKey, string][]).map(([k, label]) => (
                <button key={k} onClick={() => setSort(k)}
                  ...
                  style={{
                    background: sort === k ? "#FF3D7F18" : "transparent",
                    color: sort === k ? "#FF3D7F" : "rgba(26,22,18,0.30)",
                  }}>
                  {label}
                </button>
              ))}
          ...
          <div className="flex-1 overflow-y-auto">
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 gap-4 text-center px-6">
                ...
                <p className="font-hanken text-[#1A1612]/40 text-sm">Aucune organisation pour cette recherche.</p>
                <button onClick={resetAll} className="text-xs font-mono text-[#FF3D7F] hover:underline">
                ...
                {grouped.map(({ country, code, orgs }) => (
                  <div key={country}>
                    <div className="flex items-center gap-3 mb-4 sticky top-0 py-2 bg-[#FBF9F5]/95 backdrop-blur-sm -mx-5 px-5 z-[5]">
                      ...
                      <div className="flex-1 h-px bg-gradient-to-r from-[#1A1612]/8 to-transparent" />
                    </div>
...
function Footer({ count, groupCount }: { count: number; groupCount?: number }) {
  return (
    <div className="pt-6 pb-10 text-center border-t border-[#1A1612]/6 space-y-2 mt-4">
      <p className="font-mono text-[10px] text-[#1A1612]/20">
        {count} organisations{groupCount ? ` · ${groupCount} pays` : ""}
      </p>
      <a href="mailto:contact@spectrumforus.com?subject=Référencement annuaire"
        className="inline-flex items-center gap-1.5 text-xs font-mono text-[#FF3D7F]/50 hover:text-[#FF3D7F] transition-colors">
