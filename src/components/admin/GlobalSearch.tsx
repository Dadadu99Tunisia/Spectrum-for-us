  ...
  return (
    <>
      <button onClick={() => setOpen(true)}
        className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/[0.08] border border-white/[0.12] cursor-pointer hover:bg-white/[0.06] transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[12vh] px-4" style={{ background: "rgba(10,6,20,0.6)", backdropFilter: "blur(4px)" }} onClick={() => setOpen(false)}>
          <div onClick={e => e.stopPropagation()} className="w-full max-w-xl rounded-2xl overflow-hidden"
            ...
            <div className="flex items-center gap-3 px-4 h-14 border-b" style={{ borderColor: "rgba(243,234,219,0.1)" }}>
              <Search size={18} className="text-[#F3EADB]/40" />
              <input ref={inputRef} value={q}
                ...
                className="flex-1 bg-transparent outline-none text-[15px] text-[#F3EADB] placeholder-[#F3EADB]/30" />
              {loading && <span className="font-mono text-[10px] text-[#F3EADB]/30">…</span>}
            </div>
            <div className="max-h-[50vh] overflow-y-auto py-2">
              {q.trim().length < 2 ? (
                <p className="px-4 py-6 text-center font-hanken text-sm text-[#F3EADB]/30">Tape au moins 2 caractères.</p>
              ) : results.length === 0 && !loading ? (
                <p className="px-4 py-6 text-center font-hanken text-sm text-[#F3EADB]/30">Aucun résultat pour « {q} ».</p>
              ) : (
                results.map((r, i) => {
                  ...
                  return (
                    <button key={r.type + r.id} onClick={() => go(r)} onMouseEnter={() => setActive(i)}
                      ...
                      <span className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(243,234,219,0.07)" }}>
                        <Icon size={15} className="text-[#F3EADB]/60" />
                      </span>
                      <span className="flex-1 min-w-0">
                        <span className="block font-hanken text-sm text-[#F3EADB] truncate">{r.title}</span>
                        <span className="block font-mono text-[10px] text-[#F3EADB]/35 truncate">{r.subtitle}</span>
                      </span>
                      {i === active && <CornerDownLeft size={13} className="text-[#F3EADB]/30 shrink-0" />}
                    </button>
                  );
                })
              )}
