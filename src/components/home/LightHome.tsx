...
export function LightHome() {
  ...
  useEffect(() => {
    ...
    // Vue « home_boutiques » = dernière création par boutique (distinct on shop_id).
    // Indépendant du nombre d'articles : chaque boutique ressort TOUJOURS, même
    // si son dernier produit est ancien.
    createClient().from("home_boutiques")
      .select("id,name,slug,banner_url,last_images,last_image_url,count")
      .order("last_created_at", { ascending: false }).limit(8)
      .then(({ data }) => {
