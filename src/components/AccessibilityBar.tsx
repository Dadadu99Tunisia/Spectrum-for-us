    ...
    <>
      ...
      >
        ...
        <div className="flex justify-center">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2 border border-b-0 rounded-t-xl text-xs font-mono tracking-widest uppercase transition-all duration-200 ${
              hasAnyActive
                ? "bg-[#FF3D7F]/15 border-[#FF3D7F]/40 text-[#FF3D7F]"
                : "bg-[#F1ECE3] border-[#1A1612]/15 text-[#1A1612]/50 hover:text-[#1A1612]"
            }`}
          >
            {hasAnyActive && <span className="w-1.5 h-1.5 rounded-full bg-[#FF3D7F] animate-pulse" />}
        ...
        <div
          id="a11y-panel"
          role="group"
          className="bg-[#F1ECE3]/98 backdrop-blur-md border-t border-[#1A1612]/10 transition-all duration-300 overflow-hidden"
          ...
          <div className="max-w-3xl mx-auto px-4 py-4">
            <div className="flex items-start gap-6 flex-wrap">
              ...
              <div className="flex flex-col gap-1.5">
                <p className="font-mono text-[9px] tracking-widest uppercase text-[#1A1612]/30">
                  Taille du texte
                </p>
                  ...
                  {TEXT_SIZES.map(({ key, label }) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg border font-mono text-xs transition-all duration-200 ${
                        textSize === key
                          ? "border-[#FF3D7F] text-[#FF3D7F] bg-[#FF3D7F]/10"
                          : "border-[#1A1612]/15 text-[#1A1612]/40 hover:border-[#1A1612]/30 hover:text-[#1A1612]/70"
                      }`}
                    >
                      {label}
                    </button>
              ...
              {/* Séparateur */}
              <div className="w-px self-stretch bg-[#1A1612]/8 hidden sm:block" />
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-hanken transition-all duration-200 ${
                          isActive
                            ? "border-[#FF3D7F] text-[#FF3D7F] bg-[#FF3D7F]/10"
                            : "border-[#1A1612]/15 text-[#1A1612]/45 hover:border-[#1A1612]/30 hover:text-[#1A1612]/70"
                        }`}
                      >
                        <Icon size={11} aria-hidden="true" />
                        <span>{label}</span>
                        {isActive && (
                          <span className="w-1.5 h-1.5 rounded-full bg-[#FF3D7F]" aria-hidden="true" />
                        )}
                      </button>
                    );
                  })}
              ...
              {hasAnyActive && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-[#1A1612]/15 text-[#1A1612]/30 font-hanken text-xs hover:border-red-400/30 hover:text-red-400 transition-all self-end"
                >
                  <X size={11} />
                  Réinitialiser
                </button>
            ...
            {hasAnyActive && (
              <p className="mt-3 font-hanken text-[10px] text-[#1A1612]/30 text-center" aria-live="polite">
                Actif·ves : {[
