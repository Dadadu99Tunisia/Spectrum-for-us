...
export function ImageUploader({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      {label && (
        <label className="block font-mono text-[9px] tracking-wide text-[#101014]/30">
          {label}
        </label>
      )}
      ...
      <div
        className={`relative ${aspectClass} w-full rounded-2xl border-2 border-dashed transition-all cursor-pointer overflow-hidden
          ${dragOver ? "border-[#FF2DA0] bg-[#FF2DA0]/5" : "border-[#101014]/15 hover:border-[#101014]/30"}
      ...
      >
        ...
        {preview ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-4 text-center">
            {loading ? (
              <Loader2 size={24} className="text-[#FF2DA0] animate-spin" />
            ) : (
              <>
                <div className="w-12 h-12 rounded-2xl bg-[#101014]/5 flex items-center justify-center">
                  <ImageIcon size={20} className="text-[#101014]/20" />
                </div>
                <div>
                  <p className="font-hanken text-sm text-[#101014]/40">
                    <span className="text-[#FF2DA0]">Choisir</span> ou glisser-déposer
                  </p>
                  {hint && <p className="font-mono text-[9px] text-[#101014]/20 mt-1">{hint}</p>}
                </div>
              </>
            )}
          </div>
...
export function MultiImageUploader({
  ...
  return (
    <div className="space-y-2">
      {label && (
        <label className="block font-mono text-[9px] tracking-wide text-[#101014]/30">
          {label} <span className="text-[#101014]/20 normal-case">({values.length}/{max})</span>
        </label>
      )}

      <div className="grid grid-cols-3 gap-2">
        {values.map((url, i) => (
          <div key={url} className="relative aspect-square rounded-xl overflow-hidden border border-[#101014]/10 group">
        ...
        {values.length < max && (
          <button
            onClick={() => inputRef.current?.click()}
            disabled={loading}
            className="aspect-square rounded-xl border-2 border-dashed border-[#101014]/15 hover:border-[#FF2DA0]/40 flex flex-col items-center justify-center gap-1 transition-colors disabled:opacity-40"
          >
            {loading
              ? <Loader2 size={18} className="text-[#FF2DA0] animate-spin" />
              : <><Upload size={18} className="text-[#101014]/25" /><span className="font-mono text-[9px] text-[#101014]/25">Ajouter</span></>
            }
          </button>
        )}
      </div>
